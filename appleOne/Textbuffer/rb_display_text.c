...
void rb_display_process_ascii_key(int ch) {
    ...
    if (ch == RET_KEY_ENTER) {
        memset(d->line_buffer, ' ', d->text_cols);

        for (int x = 0; x < d->cursor_col; x++) {
            if (d->text_buffer[d->cursor_row * d->text_cols + x] != '\0') {
                d->line_buffer[x] = d->text_buffer[d->cursor_row * d->text_cols + x];
            }
        }

        d->line_buffer[d->cursor_col] = '\0';

        rb_text_new_line();
    }
    else if (ch == RET_KEY_BACKSPACE) {
        rb_display_cursor_delete_char();
    }
    else if (ch == RET_KEY_ARROW_LEFT) {
        rb_display_cursor_left();
    }
    else if (ch == RET_KEY_ARROW_RIGHT) {
        rb_display_cursor_right();
    }
    else if (ch == RET_KEY_ARROW_UP) {
        rb_display_cursor_up();
    }
    else if (ch == RET_KEY_ARROW_DOWN) {
        rb_display_cursor_down();
    }
    else if (ch >= 32) {
        if (ch >= 'a' && ch <= 'z') {
            ch += 'A' - 'a';
